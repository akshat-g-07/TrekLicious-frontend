...
const TrekCategoryGenerator = () => {
    ...
    const seasonListSliced = seasonList.sort(() => Math.random() - 0.5).slice(0, 2);

    const difficultyList = [... new Set(Treks.map(trek => trek.difficulty))]
    const difficultyListSliced = difficultyList.sort(() => Math.random() - 0.5).slice(0, 2);
    ...
    const trekCategorySliced = trekCategory.sort(() => Math.random() - 0.5).slice(0, sliceNum);
